class ImageConstant {
  // Image folder path
  static String imagePath = 'assets/images';

  // Cart Page images
  static String imgSortDown = '$imagePath/img_sort_down.png';

  // Home Page images
  static String imgSorting = '$imagePath/img_sorting.png';

  static String imgSearch = '$imagePath/img_search.png';

  static String imgIcons8Cart641 = '$imagePath/img_icons8_cart_64_1.png';

  static String imgShirtInStore = '$imagePath/img_shirt_in_store.png';

  static String img6ShirtManufacturers6Ed72h =
      '$imagePath/img_6_shirt_manufacturers_6_ed72h.png';

  static String img68909208001116 = '$imagePath/img_6890920800_1_1_16.png';

  static String imgImage1 = '$imagePath/img_image_1.png';

  static String imgF6ac7c84E08b4 = '$imagePath/img_f6ac7c84_e08b_4.png';

  static String img39816ab501a04166x143 =
      '$imagePath/img_39816ab5_01a0_4_166x143.png';

  static String imgSettings = '$imagePath/img_settings.svg';

  static String imgF6ac7c84E08b4166x143 =
      '$imagePath/img_f6ac7c84_e08b_4_166x143.png';

  static String imgNavMenu = '$imagePath/img_nav_menu.png';

  static String imgNavNotification = '$imagePath/img_nav_notification.png';

  static String imgNavHome = '$imagePath/img_nav_home.png';

  static String imgNavContractUs = '$imagePath/img_nav_contract_us.png';

  static String imgNavProfile = '$imagePath/img_nav_profile.png';

  // CheckOut One images
  static String imgLock = '$imagePath/img_lock.png';

  // Login Page images
  static String imgEye = '$imagePath/img_eye.svg';

  static String imgCheckmark = '$imagePath/img_checkmark.svg';

  static String imgFacebook = '$imagePath/img_facebook.svg';

  static String imgInstagram1 = '$imagePath/img_instagram_1.svg';

  static String imgGooglePlus = '$imagePath/img_google_plus.png';

  static String imgLink = '$imagePath/img_link.svg';

  // Common images
  static String imgMultiply30x30 = '$imagePath/img_multiply_30x30.png';

  static String PannmLogo = '$imagePath/PannmLogo.PNG';

  static String img39816ab501a04 = '$imagePath/img_39816ab5_01a0_4.png';

  static String imageNotFound = 'assets/images/image_not_found.png';
}
